...
class RPLidar(object):
    ...
    def iter_measures(self, scan_type='normal', max_buf_meas=12000):
...
def move_servo_to_angle(angle,dest):
    delta = (dest - angle)/5.0
...
map_info = open("maps/map_info.txt",'a')
...
outfile = open("maps/map_data.txt", 'a')
try:
    print('Recording measurments... Press Crl+C to stop.')
    move_servo_to_angle(90,30)
    angle = 30
    i = 0
    j = 0
...
move_servo_to_angle(angle,90)
lidar.stop()
lidar.disconnect()
outfile.write("-\n")
map_info.write(location+','+str(j)+'\n')
outfile.close()
map_info.close()
base.close()
